...
abstract contract PrivacyToken is IZRC20, ReentrancyGuard {
    ...
    /// @notice Emitted when the privacy config URI is updated
    event PrivacyConfigURIUpdated(string oldURI, string newURI);
    ...
    /// @dev Privacy configuration URI (ERC-8086 OPTIONAL but RECOMMENDED)
    /// @custom:standard-optional Helps clients understand implementation parameters
    string private _privacyConfigURI;
     ...
     *
     * This is NOT required by ERC-8086 standard!
     *
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // Privacy Configuration URI (ERC-8086 OPTIONAL but RECOMMENDED)
    // ═══════════════════════════════════════════════════════════════════════

    /**
     * @notice Returns the URI pointing to the Privacy Configuration File
     * @return URI string (e.g., "ipfs://Qm..." or "https://...")
     * @dev Contains implementation-specific parameters for client interoperability
     */
    function privacyConfigURI() external view override returns (string memory) {
        return _privacyConfigURI;
    }

    /**
     * @notice Sets the Privacy Configuration File URI
     * @param configURI The configuration URI (can be set multiple times to update)
     * @dev Only callable by initiator (token creator)
     */
    function setPrivacyConfigURI(string calldata configURI) external override {
        if (msg.sender != initiator) revert NotInitiator();

        string memory oldURI = _privacyConfigURI;
        _privacyConfigURI = configURI;

        emit PrivacyConfigURIUpdated(oldURI, configURI);
    }
}
